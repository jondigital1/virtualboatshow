...
const TRUST: [string, string][] = [
  ...
  ["Trade-In Special", "Locked in before you arrive"],
  ...
  if (status !== "ready" || !listing || !v) {
    return (
      <>
        ...
        <section style={{ background: "#F4F1EA", padding: "clamp(60px,10vw,140px) clamp(18px,3vw,44px)", minHeight: "50vh" }}>
          <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
            {status === "loading" ? (
              ...
              <>
                ...
                <Link href="/inventory" className="btn-invert" style={{ display: "inline-block", background: "#0A2138", color: "#fff", fontWeight: 700, fontSize: 14.5, padding: "13px 24px", borderRadius: 999 }}>Back to inventory →</Link>
              </>
            )}
          </div>
        </section>
      ...
      <section style={{ background: "#F4F1EA", padding: "clamp(20px,2.5vw,32px) clamp(18px,3vw,44px) 0" }}>
        <div style={{ maxWidth: 1320, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, fontFamily: MONO, fontSize: 11.5, letterSpacing: ".1em", color: "#8595a0", flexWrap: "wrap" }}>
            ...
            {v.make && <><span>/</span><span style={{ color: "#0A2138" }}>{v.make.toUpperCase()}</span></>}
          </div>
                ...
                <div style={{ display: "flex", gap: "8px 16px", flexWrap: "wrap", marginTop: 12, fontFamily: MONO, fontSize: 12.5, color: "#3d5260" }}>
                  ...
                  {v.dock && (
                    <span style={{ display: "inline-flex", gap: 16 }}>
                      ...
                      <span style={{ color: "#0A2138" }}>{v.dock}</span>
                    </span>
                  )}
                </div>
              )}
      ...
      <section style={{ background: "#F4F1EA", padding: "clamp(20px,2.5vw,30px) clamp(18px,3vw,44px) clamp(48px,6vw,80px)" }}>
        <div style={{ maxWidth: 1320, margin: "0 auto", display: "flex", gap: "clamp(20px,2.5vw,36px)", alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 580px", minWidth: 0 }}>
            {/* GALLERY */}
            <div style={{ position: "relative", borderRadius: 18, overflow: "hidden", aspectRatio: "16/10", boxShadow: "0 24px 54px -30px rgba(10,33,56,.5)", background: PLACEHOLDER_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {photos.length > 0 ? (
                ...
                <span style={{ fontFamily: MONO, fontSize: 12, letterSpacing: ".16em", color: "rgba(10,33,56,.4)" }}>// VESSEL PHOTO</span>
              )}
              {photos.length > 0 && (
                <span style={{ position: "absolute", bottom: 12, right: 12, fontFamily: MONO, fontSize: 11, background: "rgba(8,24,41,.82)", color: "#fff", padding: "6px 11px", borderRadius: 8 }}>{Math.min(activePhoto + 1, photos.length)} / {photos.length} {photos.length === 1 ? "photo" : "photos"}</span>
              )}
            </div>
            {photos.length > 1 && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginTop: 10 }}>
                {photos.slice(0, 5).map((url, i) => (
                  <button key={url + i} onClick={() => setActivePhoto(i)} style={{ padding: 0, border: `2px solid ${activePhoto === i ? "var(--accent)" : "transparent"}`, borderRadius: 11, overflow: "hidden", aspectRatio: "4/3", cursor: "pointer", position: "relative", background: PLACEHOLDER_BG }}>
                    ...
                    {i === 4 && photos.length > 5 && (
                      <span style={{ position: "absolute", inset: 0, background: "rgba(8,24,41,.62)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: MONO, fontSize: 13, fontWeight: 700 }}>+{photos.length - 5}</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <div style={{ flex: "0 1 366px", minWidth: 280, position: "sticky", top: 80, display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "#fff", border: "1px solid rgba(11,34,56,.12)", borderRadius: 18, padding: 22, boxShadow: "0 20px 46px -30px rgba(10,33,56,.4)" }}>
              ...
              <div style={{ borderTop: v.msrp > 0 ? "1px solid rgba(11,34,56,.1)" : undefined, margin: v.msrp > 0 ? "14px 0" : "0 0 14px", paddingTop: v.msrp > 0 ? 14 : 0 }}>
                ...
                {hasPrice ? (
                  ...
                  <>
                    ...
                    <div style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 26, color: "#0A2138", letterSpacing: "-.01em", lineHeight: 1.05, marginTop: 5 }}>By appointment</div>
                  </>
                )}
              </div>
              <a href={v.buoyHref} target="_blank" rel="noopener noreferrer" className="h-brighten" style={{ display: "block", width: "100%", background: "var(--accent)", color: "#0A2138", fontWeight: 700, fontSize: 15.5, padding: 15, borderRadius: 12, border: "none", cursor: "pointer", textAlign: "center", boxSizing: "border-box" }}>See it on Buoy List →</a>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                <button onClick={scrollToRequest} className="btn-outline" style={{ flex: 1, textAlign: "center", background: "#fff", color: "#0A2138", fontWeight: 700, fontSize: 14, padding: "12px 8px", borderRadius: 12, border: "1px solid rgba(11,34,56,.18)", cursor: "pointer", fontFamily: "inherit" }}>Book a walkthrough</button>
                <button onClick={() => openModal(`/sell?embed=1&dealer=${encodeURIComponent(v.dealer)}&boat=${encodeURIComponent(name)}`, "Value your trade-in")} className="btn-outline" style={{ flex: 1, textAlign: "center", background: "#fff", color: "#0A2138", fontWeight: 700, fontSize: 14, padding: "12px 8px", borderRadius: 12, border: "1px solid rgba(11,34,56,.18)", cursor: "pointer", fontFamily: "inherit" }}>Value My Vessel</button>
              </div>
            </div>
            ...
            <div style={{ background: "#fff", border: "1px solid rgba(11,34,56,.12)", borderRadius: 18, padding: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12 }}>
                <div style={{ width: 46, height: 46, flex: "0 0 auto", borderRadius: 12, background: "#0A2138", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: DISPLAY, fontWeight: 800, fontSize: 17 }}>{dealerInitials || "B"}</div>
                <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,240px),1fr))", gap: "26px 36px", alignItems: "center" }}>
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {TERMS.map(([tv, l]) => {
              const on = term === tv;
              return <button key={tv} onClick={() => setTerm(tv)} style={{ fontFamily: MONO, fontSize: 12, padding: "9px 15px", borderRadius: 999, cursor: "pointer", background: on ? "var(--accent)" : "rgba(255,255,255,.06)", color: on ? "#0A2138" : "#fff", border: `1px solid ${on ? "var(--accent)" : "rgba(255,255,255,.3)"}` }}>{l}</button>;
            })}
          </div>
        </div>
        <div style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.14)", borderRadius: 16, padding: 26, textAlign: "center" }}>
          ...
          <Link href="#request" className="h-brighten" style={{ display: "block", marginTop: 18, background: "var(--accent)", color: "#0A2138", fontWeight: 700, fontSize: 14.5, padding: 13, borderRadius: 12, textAlign: "center" }}>Get pre-qualified →</Link>
        </div>
      </div>
